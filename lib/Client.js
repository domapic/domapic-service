...
const Client = function (service, controllerData, serviceData, abilities) {
  ...
  const controllerAbilities = abilities // TODO, remove this reassignment
  ...
  const checkAbilityCorrespondence = function (remoteAbility) {
    ...
    _.each(controllerAbilities, (ability) => {
      if (correspondence) {
        return false
      }
  ...
  const registerNewAbilities = function () {
    console.log('------------------------------- controllerAbilities');
    console.log(controllerAbilities);
    return Promise.map(controllerAbilities, registerNewAbility)
  }
