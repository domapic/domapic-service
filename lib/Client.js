...
const Client = function (service, controllerData, serviceData, serviceAbilities) {
  ...
  const checkAbilityCorrespondence = function (remoteAbility) {
    ...
    if (!correspondence) {
      return service.tracer.info(templates.compiled.deletingAbility({
  ...
  const registerAbilities = function () {
    return client.get(uris.query(uris.abilities(), {
  ...
  const getApiKey = function (userData) {
    return client.get(uris.query(uris.authTokens(), {
      type: 'apiKey',
      user: userData._id
    }))
      .then(response => {
        ...
        return client.post(uris.authApiKey(), {
          user: userData._id
        }).then(response => {
