...
const ApiClient = function (service, url, apiKey) {
  ...
  const getUsers = filter => client.get(uris.query(uris.users(), filter))
  ...
  const getServices = () => client.get(uris.services())
  ...
  const getAbilities = filter => client.get(uris.query(uris.abilities(), filter))
